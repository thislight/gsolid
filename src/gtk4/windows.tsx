...
export const Window: Component<WindowProps & { open: boolean }> = (props) => {
  ...
  const ref: Gtk.Window = createWidget(Gtk.Window, rest);
...
export function useWindowSize(): WindowGeometry {
  ...
  trackGSignal(
    ...
    window.connect("realize", () => {
      ...
      if (surface == null) {
        throw new ReferenceError("window is not associated with a GdkSurface");
      }
...
export function createWindowFrameClock() {
  const window = useWindow();
  const surface = window.get_surface();
  const [clk, setClk] = createSignal<Gdk.FrameClock>(surface.get_frame_clock());
